...
func reload() -> void:
	...
	for i in range(_inventory_system.items.size()):
		...
		if _inventory_system.items[i] != null:
			slot_scene.set_item(_inventory_system.items[i])
		
		slot_scene.on_slot_clicked.connect(func(input_event: InputEvent) -> void:
...
func get_slot(index: int) -> UISlot:
	if index >= _slots_container.get_child_count():
		push_error("UIInventory.get_slot: Index out of range")
		return null
	
	return _slots_container.get_child(index)
